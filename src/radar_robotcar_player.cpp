...
void CameraCalibration::read_calibration(sensor_msgs::CameraInfo &info,
                                         std::string filename) {
...
mono::mono(const std::string &sdk_path, const std::string &dataset_path,
           ros::NodeHandle node)
  ...
  ROS_INFO("Average frametime for mono cams: %fms (frequency: %fHz)",
           avg_frametime_nsecs / 1e6, 1 / (avg_frametime_nsecs / 1e9));

  CameraCalibration mono_left_cali, mono_right_cali, mono_rear_cali;
  mono_left_cali.read_calibration(info_mono_left,
                                  models_path + "mono_left.txt");
  mono_right_cali.read_calibration(info_mono_right,
                                   models_path + "mono_right.txt");
  mono_rear_cali.read_calibration(info_mono_rear,
                                  models_path + "mono_rear.txt");
...
void mono::publish(std::string cam) {
  ...
  ros::Publisher pub_caminfo =
      nh.advertise<sensor_msgs::CameraInfo>(info_topic, 10);
...
stereo::stereo(const std::string &sdk_path, const std::string &dataset_path,
               ros::NodeHandle nh)
  ...
  ROS_INFO("Average frametime for Bumblebee stereo cam: %fms (frequency: %fHz)",
           avg_frametime_nsecs / 1e6, 1 / (avg_frametime_nsecs / 1e9));
  ...
  // wide model
  wide_left_cali.read_calibration(info_wide_left,
                                  models_path + "stereo_wide_left.txt");
  wide_right_cali.read_calibration(info_wide_right,
                                   models_path + "stereo_wide_right.txt");

  // narrow model
  narrow_left_cali.read_calibration(info_narrow_left,
                                    models_path + "stereo_narrow_left.txt");
  narrow_right_cail.read_calibration(info_narrow_right,
                                     models_path + "stereo_narrow_right.txt");
  ...
  info_narrow_left.height = info_narrow_right.height = info_wide_left.height =
      info_wide_right.height = 960;
  info_narrow_left.width = info_narrow_right.width = info_wide_left.width =
      info_wide_right.width = 1280;
}
...
void stereo::publish() {
  ...
  ros::Publisher left_img_pub =
      nh.advertise<sensor_msgs::Image>("stereo/left", 10);
  ros::Publisher centre_img_pub =
      nh.advertise<sensor_msgs::Image>("stereo/centre", 10);
  ros::Publisher right_img_pub =
      nh.advertise<sensor_msgs::Image>("stereo/right", 10);

  ros::Publisher left_wide_info_pub =
      nh.advertise<sensor_msgs::CameraInfo>("stereo/info/wide/left", 10);
  ros::Publisher right_wide_info_pub =
      nh.advertise<sensor_msgs::CameraInfo>("stereo/info/wide/right", 10);

  ros::Publisher left_narrow_info_pub =
      nh.advertise<sensor_msgs::CameraInfo>("stereo/info/narrow/left", 10);
  ros::Publisher right_narrow_info_pub =
      nh.advertise<sensor_msgs::CameraInfo>("stereo/info/narrow/right", 10);

  ROS_INFO("Will now begin publishing stereo images on: %s, %s, %s",
           left_img_pub.getTopic().c_str(), centre_img_pub.getTopic().c_str(),
           right_img_pub.getTopic().c_str());
...
lidar::lidar(const std::string &dataset_path, ros::NodeHandle nh) : nh(nh) {
  ...
  if (infile_left.is_open())
    std::cout << "Reading velodyne_left timestamps from this txt." << std::endl;
  ...
  read_timestamps(dataset_path + "/velodyne_left.timestamps", stamps_left_str,
                  stamps_left_ros);
  read_timestamps(dataset_path + "/velodyne_right.timestamps", stamps_right_str,
                  stamps_right_ros);

  avg_frametime_nsecs = (get_average_frametime_nsecs(stamps_left_ros) +
                         get_average_frametime_nsecs(stamps_right_ros)) /
                        2;

  ROS_INFO("Average frametime for Velodyne 3D LiDARs: %fms (frequency: %fHz)",
           avg_frametime_nsecs / 1e6, 1 / (avg_frametime_nsecs / 1e9));
}
...
void lidar::publish(std::string left_or_right) {
  ...
  if (left_or_right == "left") {
    ...
    throw std::invalid_argument(
        "Lidar thread started with invalid sensor selector");
  }
  ...
  while (i < stamps_str.size() && ros::ok()) {
    ...
    for (size_t i = 0; i < num_entries; ++i)
      if (file.good() && !file.eof())
        file.read((char *)&points[i].y, sizeof(float));
    for (size_t i = 0; i < num_entries; ++i)
      if (file.good() && !file.eof())
        file.read((char *)&points[i].z, sizeof(float));
    for (size_t i = 0; i < num_entries; ++i)
      if (file.good() && !file.eof())
        file.read((char *)&points[i].intensity, sizeof(float));
...
void read_timestamps(const std::string &filename,
  ...
  timestamps_rostime.erase(timestamps_rostime.begin(),
                           timestamps_rostime.begin() + clip_idx);
  timestamps_str.erase(timestamps_str.begin(),
                       timestamps_str.begin() + clip_idx);
}
...
size_t get_idx_after_seconds(const std::vector<ros::Time> &timestamps,
  ...
  if (time_passed < clip_time) {
    ...
    throw std::invalid_argument(
        "get_idx_after_seconds: clip time was longer than given timelist");
  }
  return i;
}
